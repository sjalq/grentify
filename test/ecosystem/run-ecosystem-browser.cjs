#!/usr/bin/env node
/**
 * Port a seeded sample of browser-platform Elm packages and require each run
 * to exit 0 with verification. Qualifying = direct deps ⊆ browser platform set
 * and at least one browser-surface dependency (see packages-browser.json).
 */
...
const cache = path.join(root, ".test-cache", "ecosystem-browser", "cache");
const outRoot = path.join(root, ".test-cache", "ecosystem-browser", "out");
...
for (const pkg of packages) {
  ...
  const result = spawnSync(
    process.execPath,
    [cli, input, "--out", out, "--cache", cache, "--platform", "browser"],
    {
      cwd: root,
      encoding: "utf8",
      env: process.env,
...
if (failed > 0) {
  console.error(
    `\nFAIL: ${failed}/${packages.length} browser ecosystem packages did not port cleanly`,
  );
  process.exit(1);
}
